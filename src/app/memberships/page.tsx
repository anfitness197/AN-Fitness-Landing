    ...
    <div className="relative min-h-screen bg-zinc-950 flex flex-col justify-between overflow-x-hidden text-white pt-20 sm:pt-24 pb-12 sm:pb-16">
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 flex-1 flex flex-col gap-6 sm:gap-10">
        ...
        <section className="flex flex-col gap-4 sm:gap-6">
          <div className="text-left flex flex-col gap-2 sm:gap-3">
            ...
            <p className="text-zinc-500 text-xs sm:text-sm md:text-base max-w-xl font-light">
              Current deals you can claim now on WhatsApp.
            </p>
          </div>
          <OffersCarousel />
        </section>
        ...
        <div className="text-left flex flex-col gap-2 sm:gap-3 border-t border-zinc-900 pt-8 sm:pt-10">
          ...
          <p className="text-zinc-500 text-xs sm:text-sm md:text-base max-w-xl font-light">
            Pick a plan that fits you. No signup fees, no hidden lock-ins. Forge your steel with us.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <LoadError
            message="We couldn't load membership plans. Please try again."
            onRetry={() => setRetryKey((k) => k + 1)}
          />
        ) : plans.length === 0 ? (
          <EmptyState
            title="No plans available yet"
            description="Check back soon, or contact us on WhatsApp to join."
          />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8 items-stretch mt-4 sm:mt-6">
            {plans.map((plan) => {
              ...
              return (
                ...
                >
                  {plan.popular === 1 && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-brandRed text-white text-[11px] sm:text-xs font-mono font-black tracking-widest px-3 sm:px-4 py-1 sm:py-1.5 rounded-full uppercase shadow-md shadow-brandRed/20">
                  ...
                  <div className="mt-4 sm:mt-8">
                    <a
                      href={getMembershipWhatsappUrl(plan.name, plan.price)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`w-full flex items-center justify-center gap-2 border hover:border-white px-4 py-3 sm:py-4 rounded-full text-xs sm:text-sm font-black tracking-widest uppercase transition-all duration-300 shadow-lg text-center cursor-pointer ${
                        plan.popular
        ...
        <div className="mt-10 sm:mt-16 bg-zinc-900/10 border border-zinc-900 rounded-2xl sm:rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 backdrop-blur-sm">
          ...
          <Link href="/contact">
            <RainbowButton className="px-6 sm:px-8 py-3 sm:py-3.5 text-xs sm:text-sm font-black tracking-widest uppercase">
              Contact us
            </RainbowButton>
          </Link>
        </div>
