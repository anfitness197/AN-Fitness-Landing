...
import { getWhatsappUrl as getWhatsappUrlFromLib } from "@/lib/contact";
...
export const OffersCarousel: React.FC = () => {
  ...
  const getWhatsappUrl = (text: string) => {
    return getWhatsappUrlFromLib(text);
  };
      ...
      {announcement && announcement.active === 1 && announcement.text && (
        <div className="w-full flex justify-center px-4">
          <div className="relative group overflow-hidden p-[1px] rounded-full bg-zinc-900 border border-zinc-800 flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2 sm:py-2.5 max-w-xl mx-auto shadow-2xl backdrop-blur-md">
            ...
            {announcement.badge && (
              <span className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-brandRed bg-brandRed/10 px-2 py-0.5 rounded shrink-0">
                {announcement.badge}
              </span>
            )}
            <p className="text-xs sm:text-sm text-zinc-300 font-black uppercase tracking-wider sm:tracking-widest leading-none pr-1">
              {announcement.text}
            </p>
          </div>
        </div>
                ...
                return (
                  ...
                  >
                    <div className="relative h-full flex flex-col justify-between bg-zinc-900/10 border border-zinc-900/80 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 transition-all duration-300 hover:border-zinc-800 bg-gradient-to-b from-zinc-900/30 to-transparent backdrop-blur-sm shadow-xl group">
                      <div>
                        {offer.badge && (
                          <span className="inline-block text-[11px] sm:text-xs font-mono tracking-widest text-brandRed bg-brandRed/10 border border-brandRed/20 px-2.5 sm:px-3 py-1 rounded-full uppercase mb-4 sm:mb-6 font-bold">
                            {offer.badge}
                          </span>
                        )}
                        ...
                        <div className="flex items-baseline gap-2 mb-3 sm:mb-4">
                          ...
                          <span className="text-[11px] sm:text-xs text-zinc-500 uppercase tracking-widest font-mono">{offer.subtitle}</span>
                        </div>
                      ...
                      <div className="mt-3 sm:mt-4">
                        <a
                          ...
                          className="w-full flex items-center justify-center gap-2 border border-brandRed hover:border-white bg-brandRed/10 hover:bg-brandRed text-white px-4 py-2.5 sm:py-3 rounded-full text-xs sm:text-sm font-black tracking-widest uppercase transition-all duration-300 shadow-lg text-center"
                          draggable={false}
                        >
                          <svg
